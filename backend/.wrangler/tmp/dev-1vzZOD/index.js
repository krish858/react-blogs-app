...
// .wrangler/tmp/bundle-6SW4xN/checked-fetch.js
...
// .wrangler/tmp/bundle-6SW4xN/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-6SW4xN/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
// ../common/src/index.ts
var SingupInput = z.object({
  username: stringType().email(),
  password: stringType().min(8),
  name: stringType()
});
var SinginInput = z.object({
  username: stringType().email(),
  password: stringType().min(8)
});
...
userRouter.post("/signin", async (c) => {
  const body = await c.req.json();
  const sucess = SinginInput.safeParse(body);
  if (!sucess.success) {
    return c.text("invalid inputs");
  }
  const prisma = new import_edge.PrismaClient({
